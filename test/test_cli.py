...
from brewblox_ctl_lib import cli
...
def test_wait_timeout(mocker, mock_requests, mock_retry_interval):
    ...
    mock_requests.get.return_value.raise_for_status.side_effect = cli.ConnectionError
...
def test_http_wait(mock_requests, mock_wait):
    runner = CliRunner()
    result = runner.invoke(cli.http, ['wait', 'url'])
...
def test_http_methods(mock_requests):
    runner = CliRunner()
    result = runner.invoke(cli.http, ['get', 'url'])
    ...
    runner.invoke(cli.http, ['post', 'url'])
...
def test_http_body(mock_requests):
    ...
    runner = CliRunner()
    runner.invoke(cli.http, ['post', 'url', '-d', json.dumps(body)])
    runner.invoke(cli.http, ['post', 'url', '-d', json.dumps(body), '--json-body=false'])
...
def test_http_body_conflict(mock_requests):
    runner = CliRunner()
    result = runner.invoke(cli.http, ['post', 'url', '-d', 'text', '-f', 'myfile.json'])
    assert result.exit_code != 0
    assert mock_requests.post.call_count == 0
...
def test_http_file_body(mocker, mock_requests):
    ...
    runner = CliRunner()
    runner.invoke(cli.http, ['post', 'url', '-f', 'file.json'])
    runner.invoke(cli.http, ['post', 'url', '-f', 'file.json', '--json-body=false'])
